...
with genome.open_textfile(infile) as vcf_in, open(snv_out, 'w') as snv_out, open(indel_out, 'w') as indel_out:
    ...
    while line_i.startswith('##'):
        if line_i.startswith('##FORMAT'):
            line_i = re.sub(r'Number=R', 'Number=G', line_i)
            line_i = re.sub(r'Number=A', 'Number=G', line_i)
        # if line_i.startswith('##SAMPLE') and 'NORMAL' in line_i:
        #     normal_name = line_i.split(',')[1].replace('SampleName=','')
        #     print(normal_name)
            
        # if line_i.startswith('##SAMPLE') and 'TUMOR' in line_i:
        #     tumor_name = line_i.split(',')[1].replace('SampleName=','')
        #     print(tumor_name)
            
        if line_i.startswith('##INFO=<ID=SOR,'):
            line_i = re.sub(r'Float', 'String', line_i)
        #print(normal_name,tumor_name)
    ...
    header = line_i.split('\t')
    if header[9].endswith('_T'):
        header[9] = tumor_name
        header[10] = normal_name
    else:
        header[10] = tumor_name
        header[9] = normal_name
    #header[header.index('TUMOR')] = tumor_name
    #header[header.index('NORMAL')] = normal_name
    print(header)
    snv_out.write('\t'.join(header) + '\n' )
    indel_out.write('\t'.join(header) + '\n' )
    ...
    while line_i:
        ...
        #print(vcf_i.altbase,vcf_i.refbase)

        if ',' not in vcf_i.altbase:
            #vcf_i.info = vcf_i.info+';filter='+vcf_i.filters.replace(';','&')
            #print(vcf_i.info)
            #line_i_new = '\t'.join([vcf_i.chromosome, str(vcf_i.position), vcf_i.identifier, vcf_i.refbase, vcf_i.altbase, vcf_i.qual, vcf_i.filters, vcf_i.info, vcf_i.samples[0],vcf_i.samples[1]])
