...
parser.add_argument('-g','--group',type=int,default=9,help='Level1 cutoff')
...
for i in vcf_file.columns:
    ...
    if i.endswith('.T') or '_HCC1395_' in i:
        aim_header.append(i)
...
if vcf_type == 'SEQC2':
    ...
    #对样本重复进行打分
    samples = [i for i in vcf_file.columns if (i.endswith('.T') or '_HCC1395_' in i)]
    groups = list(set([i.rsplit('_',1)[0] for i in samples]))
    group_new = []
    for g in groups:
